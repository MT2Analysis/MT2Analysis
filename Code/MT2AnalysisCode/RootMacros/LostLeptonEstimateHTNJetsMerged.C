...
//use via root -l -b -q LostLeptonEstimateHTNJetsMerged.C++

using namespace std;

void load(const char* filename = "samples/datasamples/samples_2141_dataonly.dat");
void LostLeptonEstimateHTNJetsMerged();
...
void GetLostLeptonEstimate(map<string, TH1D*> histograms, map<string, TEfficiency*> tefficiencies, double rel_sys_uncert,  double rel_sys_uncert_bg, int version, Bool_t PrintSummaryTable, Bool_t PrintYieldTable, Bool_t PrintPredictionCard, Bool_t makeFullPrintout=false);
...
Bool_t allr = false;//do estimate after summing up all HT, NJets, and NBJets regions
Bool_t allj = true;//do estimate after summing up all HT, and NJets regions
Bool_t allb = false;//do estimate after summing up all HT, and NBJets regions
...
Bool_t fISRreweight2j3jonly= false;//works only if fISRreweight=true, do 'ISR reweighting' only for events with 2 or 3 jets. Reasons for that given in AN-2013/215
...
//note that this array is longer than in standard version
...
//Note that this function is basically the same as LostLeptonEstimate.C
//but the estimate is done without HT binning, without NJets and/or NBJets binning
//therefore no detailed comments are given - as they are the given already in LostLeptonEstimate.C
//This code also saves pull distributions (which are summarizing these test estimations)
void LostLeptonEstimateHTNJetsMerged(){
	...
	outputdir = outputdir + "METHTmerged/";
	...
	cutStreamBase << " " 
      << "misc.PassJet40ID ==1"                      << "&&"
      << "(misc.HBHENoiseFlag == 0 || misc.ProcessID==10)"  << "&&" // signal (i.e. fastsim) has no flag
	...
	load(samples.Data());
   	...
   	for(size_t i = 0; i < fSamples.size(); ++i){
		...
		if(fMT2tree->misc.HT>=750.&&fMT2tree->misc.HT<1200.&&fMT2tree->NJetsIDLoose40==2&&fMT2tree->NBJets40CSVM>=1&&fMT2tree->misc.MT2>=135.&&fMT2tree->misc.MT2<=170.&&sampletype=="SingleTop"&&fMT2tree->misc.LumiSection==149&&fMT2tree->misc.Event==44699) continue;//sample T_t_highHT
		...
		if(fISRreweight && !fMT2tree->misc.isData){
			...
			if(sampletype=="WJets"){
				...
				if((fSamples[i].name).Contains("tW")){//t + W
					...
					if(!Wf){//this might be wrong, influence negligible anyway
						for(int ngl = 0; ngl<fMT2tree->NGenLepts; ++ngl){
							int id    = abs(fMT2tree->genlept[ngl].ID);
							int mid   = abs(fMT2tree->genlept[ngl].MID);//from b
							int gmid  = abs(fMT2tree->genlept[ngl].GMID);//from b
	...
	cout << "Saving." << endl;
	if(fISRreweight2j3jonly) outputname = "ISR2j3j_restnot_" + outputname;
	...
	GetLostLeptonEstimate(histos, teff, frel_sys_uncert,  frel_sys_uncert_bg, 0, true, true, true, true);
	...
	delete fLogStream;
}//LostLeptonEstimateHTNJetsMerged()
...
//version == 1,3: with e
void GetLostLeptonEstimate(map<string, TH1D*> histograms, map<string, TEfficiency*> tefficiencies, double rel_sys_uncert,  double rel_sys_uncert_bg, int version, Bool_t PrintSummaryTable, Bool_t PrintYieldTable, Bool_t PrintPredictionCard, Bool_t makeFullPrintout){
	...
	if(sqrt(pow(pred_error_stat,2)+pow(pred_error_sys,2)+pow(nW_leptveto_scaled_err,2))>0 && pred>0) hPullData->Fill((pred-nW_leptveto_scaled)/sqrt(pow(pred_error_stat,2)+pow(pred_error_sys,2)+pow(nW_leptveto_scaled_err,2)));
	if(sqrt(pow(pred_MC_error_stat,2)+pow(pred_MC_error_sys,2)+pow(nW_leptveto_scaled_err,2))>0) hPullMC  ->Fill((pred_MC-nW_leptveto_scaled)/sqrt(pow(pred_MC_error_stat,2)+pow(pred_MC_error_sys,2)+pow(nW_leptveto_scaled_err,2)));
	...
	if(fISRreweight2j3jonly) pullname = "Pulls_ISR2j3j_restnot.root";
