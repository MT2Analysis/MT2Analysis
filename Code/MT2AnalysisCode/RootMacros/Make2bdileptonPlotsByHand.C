...
//run via root -l -b -q Make2bdileptonPlotsByHand.C

using namespace std;

void load(const char* filename = "samples/datasamples/samples_2141_dataonly.dat");
void Make2bdileptonPlotsByHand();
...
//standard struct that combines MT2trees with necessary information
struct sample{
	TString name;
	TString sname;
	TString shapename;
...
const int signalregionsize = 3;
string signal_region[signalregionsize] = {"2b", "ge3b", "ge2b"};
...
Bool_t fbTagReweight = true;//BTV SF weights on MC
...
TString  outputdir = "stupidplots/Z2b/";//2b
bool fSave = true;//save plots

//code creates and plots MT2 and M(ll) spectrum for >=2b yield to justify the use of simulation in Z(nunu) for that region
void Make2bdileptonPlotsByHand(){
	...
	TString outputname = "MT2llHistos2.root";//2b

	TString  samples = "samples/samples_2l_noHTdata.dat";//only dummy
	...
	for(int i3 = 0; i3<HTregionsize;     ++i3){
		...
		string mapname;
		mapname = "MT2ll" + hs;
		if(histos.count(mapname) == 0 ) histos[mapname] = new TH1D(mapname.c_str(), "", 15, 0.,750.);
		mapname = "Mll_MT2gt100" + hs;
		if(histos.count(mapname) == 0 ) histos[mapname] = new TH1D(mapname.c_str(), "", 10, 40., 140.);
	...
	TString triggerEE = "trigger.HLT_DiElectrons";
	TString triggerMM = "trigger.HLT_DiMuons";
	TString triggerEM = "trigger.HLT_EMu";
	...
	load(samples.Data());
   	...
   	for(size_t i = 0; i < fSamples.size(); ++i){
		...
		else if(fSamples[i].sname=="ZJetsToLL") sampletype = (string)"ZJets";//nonboosted
//		else if(fSamples[i].sname=="ZJetsToLL_PtZ100")	sampletype = (string)"ZJets";//boosted
		...
		//lepton selection fitting trigger requirement
		string slep = "_had";
		if(fMT2tree->NEles==2)         {
		...
		//HT and topological region selection
		string sHT;
		fMT2tree->ZllRecalculate();//add dileptons to MET and recalculate event variables like MT2
		...
		if(fMT2tree->NBJets40CSVM == 2 ) ssignal = "_2b";
		if(fMT2tree->NBJets40CSVM >= 3 ) ssignal = "_ge3b";

		double btagSF(1.), btagSFerr(0.);
		if(!fMT2tree->misc.isData){
			btagSF = fMT2tree->SFWeight.BTagCSV40ge2; btagSFerr = fMT2tree->SFWeight.BTagCSV40ge2Error;
			weight = weight * btagSF;//2b
		}

		string hh = slep+ssignal+sHT+"_"+sampletype;
		if(slep!=""&&sHT!=""&&ssignal!=""){
			//here I select all dilepton events, not only Z events
			histos["MT2ll"+hh]->Fill(fMT2tree->misc.MT2,weight);//2b
			if(slep=="_EMu"){
				if(fMT2tree->misc.MT2>100.) histos["Mll_MT2gt100"+hh]->Fill(fMT2tree->GetDiLeptonInvMass(0,0,0,10,true) ,weight);//2b
			} else {
				if(fMT2tree->misc.MT2>100.) histos["Mll_MT2gt100"+hh]->Fill(fMT2tree->GetDiLeptonInvMass(0,1,0,10,true) ,weight);//2b
			}
		}
	} //while(myEvtList->GetEntry(counter++) !=-1)
	delete fMT2tree;
	...
	for(int i3 = 0; i3<HTregionsize;        ++i3){
		...
		histos["MT2ll"+hsl]->Add(histos["MT2ll"+hse],1);
		histos["MT2ll"+hsl]->Add(histos["MT2ll"+hsm],1);
		histos["MT2ll"+hsl]->Add(histos["MT2ll"+hsem],1);
		histos["Mll_MT2gt100"+hsl]->Add(histos["Mll_MT2gt100"+hse],1);
		histos["Mll_MT2gt100"+hsl]->Add(histos["Mll_MT2gt100"+hsm],1);
		histos["Mll_MT2gt100"+hsl]->Add(histos["Mll_MT2gt100"+hsem],1);
	}}}
	...
	for(int i4 = 0; i4<leptontypesize;   ++i4){
		...
		histos["MT2ll"+hsa]->Add(histos["MT2ll"+hsl],1);
		histos["MT2ll"+hsa]->Add(histos["MT2ll"+hsm],1);
		histos["MT2ll"+hsa]->Add(histos["MT2ll"+hsh],1);
		histos["Mll_MT2gt100"+hsa]->Add(histos["Mll_MT2gt100"+hsl],1);
		histos["Mll_MT2gt100"+hsa]->Add(histos["Mll_MT2gt100"+hsm],1);
		histos["Mll_MT2gt100"+hsa]->Add(histos["Mll_MT2gt100"+hsh],1);
	}}}
	cout << "add NBJet regions" << endl;
	for(int i1 = 0; i1<sampletypesize;   ++i1){
	for(int i3 = 0; i3<HTregionsize;        ++i3){
	for(int i4 = 0; i4<leptontypesize;   ++i4){
		string hs2 = string("_") + lepton_type[i4] + string("_") + "2b" + string("_") + HT_region[i3] + string("_") + sample_type[i1];
		string hs3 = string("_") + lepton_type[i4] + string("_") + "ge3b" + string("_") + HT_region[i3] + string("_") + sample_type[i1];
		string hsa = string("_") + lepton_type[i4] + string("_") + "ge2b" + string("_") + HT_region[i3] + string("_") + sample_type[i1];
		histos["MT2ll"+hsa]->Add(histos["MT2ll"+hs2],1);
		histos["MT2ll"+hsa]->Add(histos["MT2ll"+hs3],1);
		histos["Mll_MT2gt100"+hsa]->Add(histos["Mll_MT2gt100"+hs2],1);
		histos["Mll_MT2gt100"+hsa]->Add(histos["Mll_MT2gt100"+hs3],1);
	}}}
	...
	for(int i4 = 0; i4<leptontypesize;   ++i4){
		...
		string hsm   = string("_") + lepton_type[i4] + string("_") + signal_region[i2] + string("_") + HT_region[i3] + string("_mc");
			histos[("MT2ll"+hsm)]->Add(histos[("MT2ll"+hsq)],  1);
			histos[("MT2ll"+hsm)]->Add(histos[("MT2ll"+hsw)],  1);
			histos[("MT2ll"+hsm)]->Add(histos[("MT2ll"+hsz)],  1);
			histos[("MT2ll"+hsm)]->Add(histos[("MT2ll"+hst)],  1);
			histos[("MT2ll"+hsm)]->Add(histos[("MT2ll"+hso)],  1);
			histos[("Mll_MT2gt100"+hsm)]->Add(histos[("Mll_MT2gt100"+hsq)],  1);
			histos[("Mll_MT2gt100"+hsm)]->Add(histos[("Mll_MT2gt100"+hsw)],  1);
			histos[("Mll_MT2gt100"+hsm)]->Add(histos[("Mll_MT2gt100"+hsz)],  1);
			histos[("Mll_MT2gt100"+hsm)]->Add(histos[("Mll_MT2gt100"+hst)],  1);
			histos[("Mll_MT2gt100"+hsm)]->Add(histos[("Mll_MT2gt100"+hso)],  1);
	}}}
	...
	for(int i3 = 0; i3<HTregionsize;     ++i3){
		...
		if(sample_type[is]!="data"){
			histos["MT2ll"+hs] ->SetFillColor(colour);
			histos["MT2ll"+hs] ->SetLineColor(colour);
			histos["Mll_MT2gt100"+hs] ->SetFillColor(colour);
			histos["Mll_MT2gt100"+hs] ->SetLineColor(colour);
		} else{
			histos["MT2ll"+hs]->SetLineColor(kBlack);
			histos["MT2ll"+hs]->SetMarkerStyle(20);
			histos["MT2ll"+hs]->SetMarkerColor(kBlack);
			histos["Mll_MT2gt100"+hs]->SetLineColor(kBlack);
			histos["Mll_MT2gt100"+hs]->SetMarkerStyle(20);
			histos["Mll_MT2gt100"+hs]->SetMarkerColor(kBlack);
		}
	}}}}
	//define stacks for plots
	...
	for(int i2 = 0; i2<signalregionsize; ++i2){
	   	for(int il = 0; il<leptontypesize; ++il){
			...
			if(stacks.count("MT2ll"+h)==0) stacks["MT2ll"+h ] = new THStack(("MT2ll" +h).c_str(), ("MT2ll" +h).c_str());//2b
			if(stacks.count("Mll_MT2gt100"+h)==0) stacks["Mll_MT2gt100"+h ] = new THStack(("Mll_MT2gt100" +h).c_str(), ("Mll_MT2gt100" +h).c_str());//2b
			//stack filling
		   	for(int is = 0; is<sampletypesize; ++is){
				...
				string hs = string("_") + lepton_type[il] + string("_") + signal_region[i2] + string("_") + HT_region[i3] + string("_") + sample_type[is];
				if(histos.count("MT2ll"+hs)!=0){
				stacks["MT2ll"+h] ->Add(histos["MT2ll"+hs]);}
				if(histos.count("Mll_MT2gt100"+hs)!=0){
				stacks["Mll_MT2gt100"+h] ->Add(histos["Mll_MT2gt100"+hs]);}
			}
		}
	...
	if(leggy){
   	for(int is = 0; is<sampletypesize; ++is){
		string hs = string("_") + string("LL_ge2b_allHT") + string("_") + sample_type[is];
		if(sample_type[is]=="mc") continue;
		//legend filling
		if(sample_type[is]!="data" && sample_type[is]!="QCD" && sample_type[is]!="WJets") Legend1->AddEntry(histos["MT2ll"+hs], (sample_type[is]).c_str(), "f");//2b
		else if(sample_type[is]=="data") Legend1->AddEntry(histos["MT2ll"+hs], (sample_type[is]).c_str(), "lp");//2b
		leggy = false;
	}
	}
	MakePlots(histos, stacks, Legend1);//make the plot
...
void MakePlots(map<string, TH1D*> histos, map<string, THStack*> stacks, TLegend *Legend1){
	...
	TString ytitle = "Events"; TString xtitle = "M_{T2} [GeV]";//2b
...
//copy from a function of MassPlotter.cc - no detailed comments
void Make1DPlotsRatio(THStack *hstack, TH1D* histmc, TH1D *histdata, bool logFlag, bool normalize, TString outname, TLegend *leg, TString xtitle, TString ytitle, float overlayScale){
...
//standard load function for reading samples.dat
