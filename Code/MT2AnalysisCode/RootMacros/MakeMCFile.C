...
//run via root -l -b -q MakeMCFile.C++
...
bool fbTagReweight  = true;//reweight MC due to BTV SF, default is true
bool TOBTECreweight = true;//reweight MC due to TOBTEC filter efficiency - this should be obsolete for 13 TeV
bool fISRreweight   = true;//reweight due to 'ISR recipe' - no default, do both and decide

//this function does the same as MakeDataFile.C, but for simulation
void MakeMCFile(){

  	gROOT->ProcessLine(".x SetStyle_PRD.C");
	TString fOutDir                    = "../Results/Filtered/";
	//file where histograms will be stored
	TString outputname = "MCNumbersISR.root";
	if(!fISRreweight) outputname = "MCNumbers.root";
	TString samples = "samples/samples_HTandMET_filter.dat";
	if(TOBTECreweight){
		outputname = "MCNumbersISR_TOBTECreweight.root";
		if(!fISRreweight) outputname = "MCNumbers_TOBTECreweight.root";
		samples = "samples/samples_HTandMET.dat";
	}
	//files for TOBTECreweighting
	...
	fCutStreamSignal << " " 
	  << "misc.MT2>=100"                                               << "&&"//including MT2 cut
	  << "misc.MET>=30"                                                << "&&"
	  << "misc.HT >=450"                                               << "&&"
	  << "NEles==0"                                                    << "&&"
	  << "NMuons==0"                                                   << "&&"
  ...
  triggerStream << "( ( "
	...
	load(samples.Data());
  	...
  	for(size_t i = 0; i < fSamples.size(); ++i){
	    ...
	    if(fSamples[i].sname=="QCD") sampletype = "QCD";
	    if(fSamples[i].shapename=="WJets") sampletype = "WJets";
	    if(fSamples[i].sname=="DY") sampletype = "ZJets";
	    if(fSamples[i].sname=="Top") sampletype = "Top";
	    if(fSamples[i].sname=="VV") sampletype = "Other";
	...
	//run over all selected events
		...
		if(TOBTECreweight){
			//do tobtec reweighting - this could be simplified but will be obsolete anyway
			...
			float eff = effmap->GetEfficiency(bin);//get tobtec weight
			float efflow = effmap->GetEfficiencyErrorLow(bin);
			float effup = effmap->GetEfficiencyErrorUp(bin); 

			if(eff>0.) weight = weight * eff;
			if(eff>0.) weightnoISR = weightnoISR * eff;
		}

		//get HT and topological region
		string sHT;
		...
		//get 'ISR weight'
		if(!fMT2tree->misc.isData && fISRreweight){
		...
		string hh = sHT + ssignal + "_" + sampletype;
		histos[(string)"MT2"        + hh]->Fill(fMT2tree->misc.MT2, weight);//fill histograms
	...
	for(int i2 = 0; i2<signalregionsize; ++i2){
	for(int i3 = 0; i3<HTbinsize;        ++i3){
		string hsq   = "MT2" + string("_") + HT_bin[i3] + string("_") + signal_region[i2] + string("_QCD");
		string hsw   = "MT2" + string("_") + HT_bin[i3] + string("_") + signal_region[i2] + string("_WJets");
		string hsz   = "MT2" + string("_") + HT_bin[i3] + string("_") + signal_region[i2] + string("_ZJets");
		string hst   = "MT2" + string("_") + HT_bin[i3] + string("_") + signal_region[i2] + string("_Top");
		string hso   = "MT2" + string("_") + HT_bin[i3] + string("_") + signal_region[i2] + string("_Other");
		string hsmc  = "MT2" + string("_") + HT_bin[i3] + string("_") + signal_region[i2] + string("_mc");
		histos[hsmc]->Add(histos[hsq],1);
		histos[hsmc]->Add(histos[hsw],1);
		histos[hsmc]->Add(histos[hsz],1);
		histos[hsmc]->Add(histos[hst],1);
		histos[hsmc]->Add(histos[hso],1);
	}}
	cout << "Saving." << endl;
