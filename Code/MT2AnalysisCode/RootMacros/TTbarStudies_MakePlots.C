...
//run via root -l -b -q TTbarStudies_MakePlots.C++
...
void TTbarStudies_MakePlots();

const int fVerbose = 3;

//same arrays as in TTbarStudies.C
...
//plots nominal and +/- 1 sigma per systematics (normalized or notnormalized)
//using histograms produced by TTbarStudies.C
void TTbarStudies_MakePlots(){
	...
	bool WnoScaleMatching = true;//see in TTbarStudies.C
	bool ISRdefault       = true;//see in TTbarStudies.C (fISRreweight)

	//number of systematics, and which systematics you want to plot
	const int numberdifferentplots = 15;
	string differentplots[numberdifferentplots] = {"Nominal", "differentMC", "JES", "MET", "Scale", "Matching", "BSF", "PU" , "ISR", "Wxs", "Topxs", "SingleTopxs", "Restxs", "TopPtReweighted", "TopPtReweightedV2"};
	bool   whattoplot[numberdifferentplots]     = {false,     true,          true,  true,  true,    true,       true,  true,   true, true,  true,    false,          false, false, false};

	//inputfile
	TString   inputdir = "Filtered/TTbarStudies/";
	TString  inputname = "TTbarStudiesHistograms_all";
	if(WnoScaleMatching)  inputname += "_noWscaleupdown";
	if(ISRdefault)        inputname += "_ISRdefault";
	inputname += ".root";

	//output directory
	TString outputdir  = "Filtered/TTbarStudies/Plots/";
	if(ISRdefault)       outputdir += "ISRdefault/";
	else                outputdir += "noISRdefault/";
	if(WnoScaleMatching) outputdir += "all_noWscaleupdown/";
	else                 outputdir += "all/";
	...
	//define nominal, down, and up histogram
	...
	for(int i3 = 0; i3<HTregionsize;     ++i3){
		...
		hs0 = "_Nominal"+hs;
		if(!WnoScaleMatching){
			if(differentplots[i1]=="Nominal"||differentplots[i1]=="JES"||differentplots[i1]=="MET"||differentplots[i1]=="BSF"||differentplots[i1]=="PU"||differentplots[i1]=="ISR"||differentplots[i1]=="Wxs"||differentplots[i1]=="Topxs"||differentplots[i1]=="SingleTopxs"||differentplots[i1]=="Restxs"||differentplots[i1]=="TopPtReweighted"||differentplots[i1]=="TopPtReweightedV2") continue;
		}
		...
		if(differentplots[i1]=="SingleTopxs") { numhistos = 2; hs1 = "_SingleTopUp"+hs;    hs2 = "_SingleTopDown"+hs; }
		if(differentplots[i1]=="Restxs")      { numhistos = 2; hs1 = "_TTVUp"+hs;          hs2 = "_TTVDown"+hs;       }
		if(differentplots[i1]=="TopPtReweighted"){numhistos=3; hs1 = "_Nominal"+hs;        hs2 = "_TopPtReweightedTwice"+hs;hs3 = "_TopPtReweighted"+hs; }
		if(differentplots[i1]=="TopPtReweightedV2"){numhistos=3;hs1= "_Nominal"+hs;        hs2="_TopPtReweightedTwiceV2"+hs;hs3="_TopPtReweightedV2"+hs; }
		if(numhistos<3) hs3 = "_Nominal"+hs;//dummy
		if(numhistos<2) hs2 = "_Nominal"+hs;//dummy
		if(numhistos<1) hs1 = "_Nominal"+hs;//dummy

		//only thing to be set is the color (nominal is black, up/down is blue/red)
		...
		if(signal_region[i2]=="2j0b")    name = name + ", 2 jets, 0 b jets";
		if(signal_region[i2]=="2j1to2b") name = name + ", 2 jets, #geq 1 b jets";
		if(signal_region[i2]=="3to5j0b") name = name + ", 3-5 jets, 0 b jets";
		if(signal_region[i2]=="3to5j1b") name = name + ", 3-5 jets, 1 b jet";
		if(signal_region[i2]=="3to5j2b") name = name + ", 3-5 jets, 2 b jets";
		if(signal_region[i2]=="6j0b")    name = name + ", #geq 6 jets, 0 b jets";
		if(signal_region[i2]=="6j1b")    name = name + ", #geq 6 jets, 1 b jet";
		if(signal_region[i2]=="6j2b")    name = name + ", #geq 6 jets, 2 b jets";
		if(signal_region[i2]=="3b")      name = name + ", #geq 3 jets, #geq 3 b jets";

		TString filename = differentplots[i1]+"_"+samplekind+"_"+ HT_region[i3] + "_"+signal_region[i2];
...
//this is a copy from a function of MassPlotter.cc - no detailed comments
void MakePlot(int numhistos, TH1D *h0_orig, TH1D *h1_orig, TH1D *h2_orig, TH1D *h3_orig, TLegend *leg, bool logflag, TString titlebox, TString name, TString outputdir, bool normalized){
	...
	if(fSave&&name.Contains("ISR")&&(name.Contains("mediumHT")||name.Contains("lowHT"))&&(name.Contains("2j0b")||name.Contains("3to5j0b"))) {
		TString nnname = outputdir+save+".C";
		c1->SaveAs(nnname.Data());
	}
	delete p_plot; delete p_ratio; delete c1;
