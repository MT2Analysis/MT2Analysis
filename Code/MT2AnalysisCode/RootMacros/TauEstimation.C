...
#include <string>
#include <cmath>
#include "/shome/haweber/MT2Analysis_8TeV/Code/MT2AnalysisCode/MT2Code/include/MT2tree.hh"//use your own path of TauEstimation.C
#include "/shome/haweber/MT2Analysis_8TeV/Code/MT2AnalysisCode/TESCO/include/helper/Utilities.hh"

//run via root -l -b -q TauEstimation.C++
...
string HT_bin[HTbinsize] = {"HTge450", "HTge750"};//dummy, will be defined by fMET,fHT
...
void TauEstimation(){
	...
	for(int i3 = 0; i3<HTbinsize;        ++i3){
		...
		for(unsigned int i0 = 0; i0<histonames.size(); ++i0){
			...
			mapname = "noISR_" + histonames[i0] + hs;//no ISR case, so that you easily can run later the TauEstimationFromRootfile.C
			if(histos.count(mapname) == 0 ) histos[mapname] = new TH1D(mapname.c_str(), "", NMT2bins, MT2bins);
		}
	}}}
	...
	//event selection, contains both lost tau and 1 tau event selection
	...
	cutStreamBase << " " 
      << "misc.PassJet40ID ==1"                      << "&&"
      << "(misc.HBHENoiseFlag == 0 || misc.ProcessID==10)"  << "&&" // for signal sample (fastsim)
	...
	//run over all selected events
		...
		//get signal region string (HT x topological region)
		string sHT;
		if(fMET){
	...
	teffnames.push_back("TEff_TauPurity");
	teffnames.push_back("TEff_TauEfficiency");
...
//TESTSINGLETOP
	...
	if(fRebin){//rebin if want to do only estimation in (HT x topological) bins
	for(int i1 = 0; i1<sampletypesize;   ++i1){
	for(int i2 = 0; i2<signalregionsize; ++i2){
	for(int i3 = 0; i3<HTbinsize;        ++i3){
		if(fMET && i3==1) continue;
...
//--> 2x2x2 = 8 cases * 2 (data/MC) = 16 implementations into 2 formula (pred and predMC)
		...
		double pred_error_sys    = sqrt(pow((numberData - bgk)*R_LL_err,2) + pow(bgkerr*R_LL,2) + pow(nWT_dTau*R_LL_err*fdoubeLLerr,2 ));
...
void PredictionFile(int version, vector<int> sr, vector<int> htr, vector<double> MT2low, vector<double> MT2up, vector<double> numtrueW, vector<double> numtrueW_bg, vector<double> numData, vector<double> numBG, vector<double> numBGLL, vector<double> LLeff, vector<double> LLefferr, vector<double> RLL, vector<double> RLLerr, vector<double> MTeff, vector<double> MTefferr, vector<double> mctruth, vector<double> mctrutherr, vector<double> MCpred, vector<double> MCpred_stat_err, vector<double> MCpred_syst_err, vector<double> datapred, vector<double> datapred_stat_err, vector<double> datapred_syst_err, Bool_t PlotPrediction){
	...
	string oldsr = "dummy";
	string oldHT = "dummy";
	vector<double> mt2binning, dp, dpe, st, ste;
	mt2binning.clear(); dp.clear(); dpe.clear(); st.clear(); ste.clear();
	for(unsigned int n = 0; n<sr.size(); ++n){
		string lep = "Tau";
		string sigreg;
