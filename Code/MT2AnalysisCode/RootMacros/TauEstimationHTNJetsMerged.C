...
//run via root -l -b -q TauEstimationHTNJetsMerged.C++

using namespace std;

void load(const char* filename = "samples/datasamples/samples_2141_dataonly.dat");
void TauEstimationHTNJetsMerged();
...
void GetTauEstimate(map<string, TH1D*> histograms, map<string, TEfficiency*> tefficiencies, double rel_sys_uncert,  double rel_sys_uncert_bg, int version, Bool_t PrintSummaryTable, Bool_t PrintYieldTable, Bool_t PrintPredictionCard, Bool_t makeFullPrintout=false);
...
Bool_t allr = false;//rebin all HT x NJets x NBJets region into one, keep MT2
Bool_t allj = true; //rebin all HT x NJets region into one, keep NBJets, MT2
Bool_t allb = false;//rebin all HT x NBJets region into one, keep NJets, MT2
...
Bool_t fISRreweight2j3jonly = false;//apply 'ISR recipe' only on 2 jet/3 jet final states, needs fISRreweight = true, try both cases
...
const int signalregionsize = 16;//enlarged, depending on rebinning
string signal_region[signalregionsize] = {"2j0b", "2j1to2b", "3to5j0b", "3to5j1b", "3to5j2b", "6j0b", "6j1b", "6j2b", "3b", "ge2j0b", "ge2j1b", "ge2jge2b", "2jge0b", "3to5jge0b", "ge6jge0b", "ge2j"};
const int HTbinsize = 1;
string HT_bin[HTbinsize] = {"HTge450"};//dummy

//This code does exactly the same as TauEstimation.C, but rebins the signal region depending on flags 'allr', 'allb', 'allj'
//therefore no detailed comments
void TauEstimationHTNJetsMerged(){
	...
	if(allr) outputdir += "allrebinned/";
	if(allj) outputdir += "alljetrebin/";
	if(allb) outputdir += "allbjrebin/";
    	Util::MakeOutputDir(outputdir);

	TString outputname = "TauEstimationHistograms.root";

	TString  samples = "samples/samples_METHTmergedLL_filter.dat";
	...
	for(int i3 = 0; i3<HTbinsize;        ++i3){
		...
		int NMT2bins;
			...
			if(signal_region[i2]=="ge2j0b")  NMT2bins = gNMT2bins_3j0b_mHT;
			if(signal_region[i2]=="ge2j1b")  NMT2bins = gNMT2bins_3j1b_mHT;
			if(signal_region[i2]=="ge2jge2b")NMT2bins = gNMT2bins_3j2b_mHT;
			if(signal_region[i2]=="2jge0b")  NMT2bins = gNMT2bins_2j0b_mHT;
			if(signal_region[i2]=="3to5jge0b")NMT2bins= gNMT2bins_3j0b_mHT;
			if(signal_region[i2]=="ge6jge0b")NMT2bins = gNMT2bins_6j0b_mHT;
  		...
  		double MT2bins[NMT2bins+1];
			...
			if(signal_region[i2]=="ge2j0b")  { for(int i0 = 0; i0<=NMT2bins; ++i0) MT2bins[i0] = gMT2bins_3j0b_mHT[i0];  }
			if(signal_region[i2]=="ge2j1b")  { for(int i0 = 0; i0<=NMT2bins; ++i0) MT2bins[i0] = gMT2bins_3j1b_mHT[i0];  }
			if(signal_region[i2]=="ge2jge2b"){ for(int i0 = 0; i0<=NMT2bins; ++i0) MT2bins[i0] = gMT2bins_3j2b_mHT[i0];  }
			if(signal_region[i2]=="2jge0b")  { for(int i0 = 0; i0<=NMT2bins; ++i0) MT2bins[i0] = gMT2bins_2j0b_mHT[i0];  }
			if(signal_region[i2]=="3to5jge0b"){for(int i0 = 0; i0<=NMT2bins; ++i0) MT2bins[i0] = gMT2bins_3j0b_mHT[i0];  }
			if(signal_region[i2]=="ge6jge0b"){ for(int i0 = 0; i0<=NMT2bins; ++i0) MT2bins[i0] = gMT2bins_6j0b_mHT[i0];  }
	...
	cutStream << " " 
		...
		<< "misc.Vectorsumpt < 70";
		  cutStream  << "&& misc.MinMetJetDPhi4Pt40 >0.3";
	          cutStream << "&&((misc.MET>200&&misc.HT<=750&&misc.HT>=450)||(misc.HT>750&&misc.MET>30))";
	...
	triggerStream << "( ( ( ( ( "
			...
			<< "trigger.HLT_PFNoPUHT350_PFMET100_v3==1 || trigger.HLT_PFNoPUHT350_PFMET100_v4==1 ) )&&misc.MET>200&&misc.HT<=750&&misc.HT>=450)";
	triggerStream << "|| ( ( "
			...
			<< "trigger.HLT_PFNoPUHT650_v1 == 1 || trigger.HLT_PFNoPUHT650_v3 == 1 || trigger.HLT_PFNoPUHT650_v4 == 1) &&misc.HT>750&&misc.MET>30 ) )&&TOBTECTagger<=8&&ExtraBeamHaloFilter==0)";
	...
	load(samples.Data());
   	...
   	for(size_t i = 0; i < fSamples.size(); ++i){
		...
		if(fISRreweight && !fMT2tree->misc.isData){
			...
			if(fISRreweight2j3jonly) {if(fMT2tree->NJetsIDLoose40<=3) weight = weight * ISRweight;}
			else { weight = weight * ISRweight;}
		}

		string sHT = "_HTge450";
		string ssignal;
		if(allj){
			if(fMT2tree->NJetsIDLoose40 >= 2 && fMT2tree->NBJets40CSVM == 0  ) ssignal = "_ge2j0b";
			if(fMT2tree->NJetsIDLoose40 >= 2 && fMT2tree->NBJets40CSVM == 1  ) ssignal = "_ge2j1b";
			if(fMT2tree->NJetsIDLoose40 >= 2 && fMT2tree->NBJets40CSVM >= 2  ) ssignal = "_ge2jge2b";
		} else if(allb){
			if(fMT2tree->NJetsIDLoose40 == 2                                 ) ssignal = "_2jge0b";
			if(fMT2tree->NJetsIDLoose40 >= 3 && fMT2tree->NJetsIDLoose40 <= 5) ssignal = "_3to5jge0b";
			if(fMT2tree->NJetsIDLoose40 >= 6                                 ) ssignal = "_ge6jge0b";
		} else ssignal = "_ge2j";

		double btagSF(1.), btagSFerr(0.);
		if(!fMT2tree->misc.isData && fbTagReweight){
			if(ssignal=="_ge2j0b"){btagSF = fMT2tree->SFWeight.BTagCSV40eq0; btagSFerr = fMT2tree->SFWeight.BTagCSV40eq0Error;}
			if(ssignal=="_ge2j1b"){btagSF = fMT2tree->SFWeight.BTagCSV40eq1; btagSFerr = fMT2tree->SFWeight.BTagCSV40eq1Error;}
			if(ssignal=="_ge2jge2b"){btagSF = fMT2tree->SFWeight.BTagCSV40ge2; btagSFerr = fMT2tree->SFWeight.BTagCSV40ge2Error;}
	...
	//			++NumRecoTaus;
		...
		if(recoedtau     ) histos[(string)"TauEvents"        + hh]->Fill(fMT2tree->misc.MT2, weight);
		if(recoedtaunomt ) histos[(string)"TauEvents_noMT"   + hh]->Fill(fMT2tree->misc.MT2, weight);
		if(recoedtau     ) histos[(string)"noISR_TauEvents"        + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
		if(recoedtaunomt ) histos[(string)"noISR_TauEvents_noMT"   + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
		if(fbTagError && !fMT2tree->misc.isData && btagSF!=0){
			if(recoedtau     ) histos[(string)"TauEvents_SFup"       + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
			if(recoedtaunomt ) histos[(string)"TauEvents_noMT_SFup"  + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
			if(recoedtau     ) histos[(string)"TauEvents_SFdown"     + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
			if(recoedtaunomt ) histos[(string)"TauEvents_noMT_SFdown"+ hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
			if(recoedtau     ) histos[(string)"noISR_TauEvents_SFup"       + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
			if(recoedtaunomt ) histos[(string)"noISR_TauEvents_noMT_SFup"  + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
			if(recoedtau     ) histos[(string)"noISR_TauEvents_SFdown"     + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
			if(recoedtaunomt ) histos[(string)"noISR_TauEvents_noMT_SFdown"+ hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		}
	...
	if(recoedtau     && genhadtau==0)            histos[(string)"TauEventsBG"           + hh]->Fill(fMT2tree->misc.MT2, weight);//bg
	if(recoedtaunomt && genhadtau==0)            histos[(string)"TauEventsBG_noMT"      + hh]->Fill(fMT2tree->misc.MT2, weight);//bg
	if(recoedtau     && genhadtau==0 && LLfound) histos[(string)"TauEventsBG_LL"        + hh]->Fill(fMT2tree->misc.MT2, weight);//bg
	if(recoedtaunomt && genhadtau==0 && LLfound) histos[(string)"TauEventsBG_noMT_LL"   + hh]->Fill(fMT2tree->misc.MT2, weight);//bg
	if(recoedtau     && LLfound)                 histos[(string)"TauEvents_LL"          + hh]->Fill(fMT2tree->misc.MT2, weight);//signal
	if(recoedtaunomt && LLfound)                 histos[(string)"TauEvents_noMT_LL"     + hh]->Fill(fMT2tree->misc.MT2, weight);//signal
	if(recoedtau     && genhadtau==2)            histos[(string)"TauEvents_dTau"        + hh]->Fill(fMT2tree->misc.MT2, weight);//signal
	if(recoedtaunomt && genhadtau==2)            histos[(string)"TauEvents_noMT_dTau"   + hh]->Fill(fMT2tree->misc.MT2, weight);//signal
	if(recoedtau     && genhadtau==0)            histos[(string)"noISR_TauEventsBG"           + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(recoedtaunomt && genhadtau==0)            histos[(string)"noISR_TauEventsBG_noMT"      + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(recoedtau     && genhadtau==0 && LLfound) histos[(string)"noISR_TauEventsBG_LL"        + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(recoedtaunomt && genhadtau==0 && LLfound) histos[(string)"noISR_TauEventsBG_noMT_LL"   + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(recoedtau     && LLfound)                 histos[(string)"noISR_TauEvents_LL"          + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(recoedtaunomt && LLfound)                 histos[(string)"noISR_TauEvents_noMT_LL"     + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(recoedtau     && genhadtau==2)            histos[(string)"noISR_TauEvents_dTau"        + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(recoedtaunomt && genhadtau==2)            histos[(string)"noISR_TauEvents_noMT_dTau"   + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(fbTagError && !fMT2tree->misc.isData && btagSF!=0){
		if(recoedtau     && genhadtau==0)            histos[(string)"TauEventsBG_SFup"         + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(recoedtaunomt && genhadtau==0)            histos[(string)"TauEventsBG_noMT_SFup"    + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(recoedtau     && genhadtau==0 && LLfound) histos[(string)"TauEventsBG_LL_SFup"      + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(recoedtaunomt && genhadtau==0 && LLfound) histos[(string)"TauEventsBG_noMT_LL_SFup" + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(recoedtau     && LLfound)                 histos[(string)"TauEvents_LL_SFup"        + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(recoedtaunomt && LLfound)                 histos[(string)"TauEvents_noMT_LL_SFup"   + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(recoedtau     && genhadtau==2)            histos[(string)"TauEvents_dTau_SFup"      + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(recoedtaunomt && genhadtau==2)            histos[(string)"TauEvents_noMT_dTau_SFup" + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(recoedtau     && genhadtau==0)            histos[(string)"TauEventsBG_SFdown"        +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(recoedtaunomt && genhadtau==0)            histos[(string)"TauEventsBG_noMT_SFdown"   +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(recoedtau     && genhadtau==0 && LLfound) histos[(string)"TauEventsBG_LL_SFdown"     +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(recoedtaunomt && genhadtau==0 && LLfound) histos[(string)"TauEventsBG_noMT_LL_SFdown"+hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(recoedtau     && LLfound)                 histos[(string)"TauEvents_LL_SFdown"       +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(recoedtaunomt && LLfound)                 histos[(string)"TauEvents_noMT_LL_SFdown"  +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(recoedtau     && genhadtau==2)            histos[(string)"TauEvents_dTau_SFdown"     +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(recoedtaunomt && genhadtau==2)            histos[(string)"TauEvents_noMT_dTau_SFdown"+hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(recoedtau     && genhadtau==0)            histos[(string)"noISR_TauEventsBG_SFup"         + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(recoedtaunomt && genhadtau==0)            histos[(string)"noISR_TauEventsBG_noMT_SFup"    + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(recoedtau     && genhadtau==0 && LLfound) histos[(string)"noISR_TauEventsBG_LL_SFup"      + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(recoedtaunomt && genhadtau==0 && LLfound) histos[(string)"noISR_TauEventsBG_noMT_LL_SFup" + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(recoedtau     && LLfound)                 histos[(string)"noISR_TauEvents_LL_SFup"        + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(recoedtaunomt && LLfound)                 histos[(string)"noISR_TauEvents_noMT_LL_SFup"   + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(recoedtau     && genhadtau==2)            histos[(string)"noISR_TauEvents_dTau_SFup"      + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(recoedtaunomt && genhadtau==2)            histos[(string)"noISR_TauEvents_noMT_dTau_SFup" + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(recoedtau     && genhadtau==0)            histos[(string)"noISR_TauEventsBG_SFdown"        +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(recoedtaunomt && genhadtau==0)            histos[(string)"noISR_TauEventsBG_noMT_SFdown"   +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(recoedtau     && genhadtau==0 && LLfound) histos[(string)"noISR_TauEventsBG_LL_SFdown"     +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(recoedtaunomt && genhadtau==0 && LLfound) histos[(string)"noISR_TauEventsBG_noMT_LL_SFdown"+hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(recoedtau     && LLfound)                 histos[(string)"noISR_TauEvents_LL_SFdown"       +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(recoedtaunomt && LLfound)                 histos[(string)"noISR_TauEvents_noMT_LL_SFdown"  +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(recoedtau     && genhadtau==2)            histos[(string)"noISR_TauEvents_dTau_SFdown"     +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(recoedtaunomt && genhadtau==2)            histos[(string)"noISR_TauEvents_noMT_dTau_SFdown"+hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
	}

	if(eventgood)               histos[(string)"WEvents"             +hh]->Fill(fMT2tree->misc.MT2, weight);
	if(eventgood_noLL)          histos[(string)"WEvents_noLL"        +hh]->Fill(fMT2tree->misc.MT2, weight);
	if(genhadtau==2)            histos[(string)"WEvents_dTau"        +hh]->Fill(fMT2tree->misc.MT2, weight);//double tau == no LL, as we have at max 2 W decays
	if(acceptance)              histos[(string)"WEventsAcc"          +hh]->Fill(fMT2tree->misc.MT2, weight);
	if(acceptance_noLL)         histos[(string)"WEventsAcc_noLL"     +hh]->Fill(fMT2tree->misc.MT2, weight);
	if(leptfound)               histos[(string)"WEventsReco"         +hh]->Fill(fMT2tree->misc.MT2, weight);
	if(leptfound&&genhadtau==2) histos[(string)"WEventsReco_dTau"    +hh]->Fill(fMT2tree->misc.MT2, weight);
	if(leptfound_noLL)          histos[(string)"WEventsReco_noLL"    +hh]->Fill(fMT2tree->misc.MT2, weight);
	if(leptfoundnomt)           histos[(string)"WEventsReco_noMT"     +hh]->Fill(fMT2tree->misc.MT2, weight);
	if(leptfoundnomt_noLL)      histos[(string)"WEventsReco_noMT_noLL"+hh]->Fill(fMT2tree->misc.MT2, weight);
	if(eventgood)               histos[(string)"noISR_WEvents"             +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(eventgood_noLL)          histos[(string)"noISR_WEvents_noLL"        +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(genhadtau==2)            histos[(string)"noISR_WEvents_dTau"        +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(acceptance)              histos[(string)"noISR_WEventsAcc"          +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(acceptance_noLL)         histos[(string)"noISR_WEventsAcc_noLL"     +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(leptfound)               histos[(string)"noISR_WEventsReco"         +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(leptfound&&genhadtau==2) histos[(string)"noISR_WEventsReco_dTau"    +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(leptfound_noLL)          histos[(string)"noISR_WEventsReco_noLL"    +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(leptfoundnomt)           histos[(string)"noISR_WEventsReco_noMT"     +hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(leptfoundnomt_noLL)      histos[(string)"noISR_WEventsReco_noMT_noLL"+hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(fbTagError && !fMT2tree->misc.isData && btagSF!=0){
		if(eventgood)               histos[(string)"WEvents_SFup"       +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(eventgood_noLL)          histos[(string)"WEvents_noLL_SFup"  +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(eventgood&&genhadtau==2) histos[(string)"WEvents_dTau_SFup"  +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(eventgood)               histos[(string)"WEvents_SFdown"     +hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(eventgood_noLL)          histos[(string)"WEvents_noLL_SFdown"+hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(eventgood&&genhadtau==2) histos[(string)"WEvents_dTau_SFdown"+hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(eventgood)               histos[(string)"noISR_WEvents_SFup"       +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(eventgood_noLL)          histos[(string)"noISR_WEvents_noLL_SFup"  +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(eventgood&&genhadtau==2) histos[(string)"noISR_WEvents_dTau_SFup"  +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(eventgood)               histos[(string)"noISR_WEvents_SFdown"     +hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(eventgood_noLL)          histos[(string)"noISR_WEvents_noLL_SFdown"+hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(eventgood&&genhadtau==2) histos[(string)"noISR_WEvents_dTau_SFdown"+hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
	}
	//add LLveto for recotaus
	if(norecotau &&  eventgood           ) histos[(string)"NoRecoButGenTauEvents"      + hh]->Fill(fMT2tree->misc.MT2, weight);
	if(norecotau &&  eventgood && LLfound) histos[(string)"NoRecoButGenTauEvents_LL"   + hh]->Fill(fMT2tree->misc.MT2, weight);
	if(norecotau &&  genhadtau==2        ) histos[(string)"NoRecoButGenTauEvents_dTau" + hh]->Fill(fMT2tree->misc.MT2, weight);
	if(norecotau &&  eventgood           ) histos[(string)"noISR_NoRecoButGenTauEvents"      + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(norecotau &&  eventgood && LLfound) histos[(string)"noISR_NoRecoButGenTauEvents_LL"   + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(norecotau &&  genhadtau==2        ) histos[(string)"noISR_NoRecoButGenTauEvents_dTau" + hh]->Fill(fMT2tree->misc.MT2, weightnoISR);
	if(fbTagError && !fMT2tree->misc.isData && btagSF!=0){
		if(norecotau &&  eventgood           ) histos[(string)"NoRecoButGenTauEvents_SFup"        + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(norecotau &&  eventgood && LLfound) histos[(string)"NoRecoButGenTauEvents_LL_SFup"     + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(norecotau &&  genhadtau==2        ) histos[(string)"NoRecoButGenTauEvents_dTau_SFup"   + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF+btagSFerr));
		if(norecotau &&  eventgood           ) histos[(string)"NoRecoButGenTauEvents_SFdown"      + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(norecotau &&  eventgood && LLfound) histos[(string)"NoRecoButGenTauEvents_LL_SFdown"   + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(norecotau &&  genhadtau==2        ) histos[(string)"NoRecoButGenTauEvents_dTau_SFdown" + hh]->Fill(fMT2tree->misc.MT2, weight/btagSF*(btagSF-btagSFerr));
		if(norecotau &&  eventgood           ) histos[(string)"noISR_NoRecoButGenTauEvents_SFup"        + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(norecotau &&  eventgood && LLfound) histos[(string)"noISR_NoRecoButGenTauEvents_LL_SFup"     + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(norecotau &&  genhadtau==2        ) histos[(string)"noISR_NoRecoButGenTauEvents_dTau_SFup"   + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF+btagSFerr));
		if(norecotau &&  eventgood           ) histos[(string)"noISR_NoRecoButGenTauEvents_SFdown"      + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(norecotau &&  eventgood && LLfound) histos[(string)"noISR_NoRecoButGenTauEvents_LL_SFdown"   + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
		if(norecotau &&  genhadtau==2        ) histos[(string)"noISR_NoRecoButGenTauEvents_dTau_SFdown" + hh]->Fill(fMT2tree->misc.MT2, weightnoISR/btagSF*(btagSF-btagSFerr));
	}
	//these histograms are not used for prediction, but for important checks like tau purity
	histos[(string)"TrueRecoTauEvents"      + hh]->Fill(fMT2tree->misc.MT2, weight*((double)recotaumatchedtrue));
	histos[(string)"FakeRecoTauEvents"      + hh]->Fill(fMT2tree->misc.MT2, weight*((double)recotaumatchedfake));
	histos[(string)"AllRecoTauEvents"       + hh]->Fill(fMT2tree->misc.MT2, weight*((double)(recotaumatchedtrue+recotaumatchedfake)));
	histos[(string)"AllRecoJetEvents"       + hh]->Fill(fMT2tree->misc.MT2, weight*((double)fMT2tree->NJetsIDLoose));
	histos[(string)"AllGenTauAccEvents"     + hh]->Fill(fMT2tree->misc.MT2, weight*((double)gentauwithinacc));
	histos[(string)"noISR_TrueRecoTauEvents"      + hh]->Fill(fMT2tree->misc.MT2, weightnoISR*((double)recotaumatchedtrue));
	histos[(string)"noISR_FakeRecoTauEvents"      + hh]->Fill(fMT2tree->misc.MT2, weightnoISR*((double)recotaumatchedfake));
	histos[(string)"noISR_AllRecoTauEvents"       + hh]->Fill(fMT2tree->misc.MT2, weightnoISR*((double)(recotaumatchedtrue+recotaumatchedfake)));
	histos[(string)"noISR_AllRecoJetEvents"       + hh]->Fill(fMT2tree->misc.MT2, weightnoISR*((double)fMT2tree->NJetsIDLoose));
	histos[(string)"noISR_AllGenTauAccEvents"     + hh]->Fill(fMT2tree->misc.MT2, weightnoISR*((double)gentauwithinacc));
	} //while(myEvtList->GetEntry(counter++) !=-1)
	delete fMT2tree;
...
//		if(fMET && i3==1) continue;
	...
	cout << "Saving." << endl;
	if(fISRreweight2j3jonly) outputname = "ISR2j3jonly_"+outputname;
...
//	GetTauEstimatehistograms,tefficiencies,rel_sys_err,rel_sys_err_bg,version,SummaryTable,YieldTable,PredictionCard,makeFullPrintout)
	GetTauEstimate(histos, teff, frel_sys_uncert, frel_sys_uncert_bg, 0, true, true, true, true);
...
//version == 0,2: with R_LL (i.e. 1-e / e)
//version == 1,3: with e
void GetTauEstimate(map<string, TH1D*> histograms, map<string, TEfficiency*> tefficiencies, double rel_sys_uncert,  double rel_sys_uncert_bg, int version, Bool_t PrintSummaryTable, Bool_t PrintYieldTable, Bool_t PrintPredictionCard, Bool_t makeFullPrintout){

	map<string, TH1D*> hists = histograms;
	map<string, TEfficiency*> teffs = tefficiencies;

	TH1D *hPullData   = new TH1D("hPullData", "", 40,-4,4);
	TH1D *hPullMC     = new TH1D("hPullMC"  , "", 40,-4,4);
	TH1D *hPullMCDa   = new TH1D("hPullMCDa", "", 40,-4,4);
...
//--> 2x2x2 = 8 cases * 2 (data/MC) = 16 implementations into 2 formula (pred and predMC)
	...
	if(sqrt(pow(pred_error_stat,2)+pow(pred_error_sys,2)+pow(mctrutherr.back(),2))>0 && pred>0) hPullData->Fill((pred-mctruth.back())/sqrt(pow(pred_error_stat,2)+pow(pred_error_sys,2)+pow(mctrutherr.back(),2)));
	if(sqrt(pow(pred_MC_error_stat,2)+pow(pred_MC_error_sys,2)+pow(mctrutherr.back(),2))>0) hPullMC  ->Fill((pred_MC-mctruth.back())/sqrt(pow(pred_MC_error_stat,2)+pow(pred_MC_error_sys,2)+pow(mctrutherr.back(),2)));
	if(sqrt(pow(pred_MC_error_stat,2)+pow(pred_MC_error_sys,2)+pow(pred_error_stat,2)+pow(pred_error_sys,2))>0 && pred>0) hPullMCDa  ->Fill((pred-pred_MC)/sqrt(pow(pred_MC_error_stat,2)+pow(pred_MC_error_sys,2)+pow(pred_error_stat,2)+pow(pred_error_sys,2)));
	...
	TString  outputdir = "TauEstimation/";
	if(allr) outputdir += "allrebinned/";
	if(allj) outputdir += "alljetrebin/";
	if(allb) outputdir += "allbjrebin/";
	TString pullname = "Pulls.root";
	if(fISRreweight) pullname = "Pulls_ISR.root";
	if(fISRreweight2j3jonly) pullname = "Pulls_ISR2j3jonly.root";
	TFile *pullfile = new TFile(outputdir+pullname,"RECREATE");
	pullfile->cd();
	hPullData->Write();
	hPullMC->Write();
	hPullMCDa->Write();
...
void SummaryTable(int version, vector<int> sr, vector<int> htr, vector<double> MT2low, vector<double> MT2up, vector<double> numtrueW, vector<double> numtrueW_bg, vector<double> numData, vector<double> numBG, vector<double> numBGLL, vector<double> LLeff, vector<double> LLefferr, vector<double> RLL, vector<double> RLLerr, vector<double> MTeff, vector<double> MTefferr, vector<double> mctruth, vector<double> mctrutherr, vector<double> MCpred, vector<double> MCpred_stat_err, vector<double> MCpred_syst_err, vector<double> datapred, vector<double> datapred_stat_err, vector<double> datapred_syst_err){
	...
	*fLogStream << "\\hline\\hline"             << endl;
	...
	for(unsigned int n = 0; n<sr.size(); ++n){
		...
		if(sr[n]==9) sigreg="$\\ge 2$j, 0b";
		if(sr[n]==10)sigreg="$\\ge 2$j, 1b";
		if(sr[n]==11)sigreg="$\\ge 2$j, $\\ge 2$b";
		if(sr[n]==12)sigreg="2j, $\\ge 0$b";
		if(sr[n]==13)sigreg="$3-5$j, $\\ge 0$b";
		if(sr[n]==14)sigreg="$\\ge 6$j, $\\ge 0$b";
		if(sr[n]==15)sigreg="$\\ge 2$j, $\\ge 0$b";

		string htreg;
		htreg = "$H_{T} \\ge 450$ GeV";
