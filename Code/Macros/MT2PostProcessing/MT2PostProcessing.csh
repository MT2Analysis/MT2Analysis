...
set HOME=`pwd`
set OUTDIR=/shome/`whoami`/MT2Analysis/MT2trees/$MT2TAG/$PRODUCTION/
set DCAPUSER=dcap://t3se01.psi.ch:22125/pnfs/psi.ch/cms/trivcat/store/user/
set SRMUSER=srm://t3se01.psi.ch:8443/srm/managerv2?SFN=/pnfs/psi.ch/cms/trivcat/store/user/
set SKIMSCRIPT=$HOME"/"$SCRIPT
set TOPWORKDIR=/scratch/`whoami`
set WORKDIR=/scratch/`whoami`/$PRODUCTION/$SAMPLE
set MERGED=/scratch/`whoami`/$PRODUCTION/$SAMPLE/$SAMPLE.root
...
#cd /shome/casal/CMSSW_5_2_5/src
...
if ( $SHLIB != "none" ) then
	...
	mkdir -pv $OUTDIR/$PREFIX/skimlogs
	cp -v $SAMPLE.root $OUTDIR/$PREFIX
	#echo "srmmkdir  $SRMUSER/$OUTDIR/$PREFIX"
	#srmmkdir  $SRMUSER/$OUTDIR/$PREFIX
	#echo "srmmkdir  $SRMUSER/$OUTDIR/$PREFIX/skimlogs"
	#srmmkdir  $SRMUSER/$OUTDIR/$PREFIX/skimlogs
        #echo "lcg-cp -b -D srmv2 file:///`pwd`/$SAMPLE.root $SRMUSER/$OUTDIR/$PREFIX/$SAMPLE.root"
        #lcg-cp -b -D srmv2 file:///`pwd`/$SAMPLE.root $SRMUSER/$OUTDIR/$PREFIX/$SAMPLE.root
	...
	cp -v $SAMPLE.skim.log $OUTDIR/$PREFIX/skimlogs/$SAMPLE.skim.log
        #echo "lcg-cp -b -D srmv2 file:///`pwd`/$SAMPLE.skim.log $SRMUSER/$OUTDIR/$PREFIX/skimlogs/$SAMPLE.skim.log"
        #lcg-cp -b -D srmv2 file:///`pwd`/$SAMPLE.skim.log $SRMUSER/$OUTDIR/$PREFIX/skimlogs/$SAMPLE.skim.log
else
	# Now merge from SE
	rm -vf $MERGED
	hadd -f $MERGED $lfiles
