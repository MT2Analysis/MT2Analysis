...
// USER INPUT -------------------------------------------
TString outputdir                 = "MassPlots/LostLepton/high/20110629/allcuts_MT2gt80/";
...
Bool_t  fTopEfficencies           = false;  // set to true to take the lepton efficiencies from top
...
struct leptonicW{
	...
	vector<double> effMC;
	vector<double> errMC;
	static const double rel_sys_uncert = 0.0;
	static const double rel_sys_uncert_bg = 0.;
	static const double Wnevents       =  5510669;
	...
	double prob(){
		...
		}else if(fMCClosure && fMakeEfficienciesPablo){
...
void getEfficiencies(Long64_t nevents){
	...
	if(fMakeEfficienciesPresel){
		...
		else        tA->init("samples_WJets/samples_WJets_Presel.dat");	
	...
	if(fIncludeTop){
		if(fMakeEfficienciesPresel) {
			...
			else        tA->init("samples_WJets//samples_Top_Presel.dat");	
		}
		else       tA->init("samples_WJets//samples_Top.dat");
...
void makePlots(){
	...
	if     (fTrigger=="HT")     {
		...
		else        samples = "samples_WJets/samples_MG_V02-01-02_golden_HTonly.dat";
	}
...
//	  << "misc.MT2 > 100 && misc.MT2<150"          << "&&"
	...
	tA->makePlot("misc.MT2"                        , ele_cuts,  -3, -11 ,  HLT, "M_{T2}"     , 15, 0, 500           ,   false,   true , true,   true,  true,  false , 1);
	tA->makePlot("misc.MT2"                        , muo_cuts,  -3, -13 ,  HLT, "M_{T2}"     , 15, 0, 500           ,   false,   true , true,   true,  true,  false , 1);
...
double get_n_events(string process, bool background, bool leptveto){
	...
	else        path = "/shome/pnef/SUSY/SUSY_macros/analyzed/RunLeptJetMultAnalyzer/MT2_V00-06-10/20110730_MC_HT300_data_nocuts/skimmed/";
	...
	else        path_data="/shome/pnef/SUSY/SUSY_macros/analyzed/RunLeptJetMultAnalyzer/MT2_V00-06-10/20110730_MC_HT300_data_nocuts/skimmed/";

	if(process == "ele"){
		chain   ->Add(path+"WJets-HT300toInf-madgraph-Z2-Fastsim-4_2_4_p1-v4_NoPU-PSI.root");
