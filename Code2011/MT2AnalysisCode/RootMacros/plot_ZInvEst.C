{
  ...
  TString B = "1b";
  //TString outF_name = "histos_ZInv-MT2b_-1_9999-Jet40-HT_700-1Mu_pt10_4450pb_NoMETCorr_"+B;
  TString outF_name = "histos_ZInv-MT2_-1_9999-Jet50-HT_1000-1Mu_pt10_4450pb_"+B;

  //TString outF_name = hfilename+"_"+B;
    cout << outF_name << endl;
    ...
    //colors[nf] = 1; legs[nf] = "LM9"; names[nf++] = "LM9";
    ...
    for(int i=0; i< nf; i++){
      ...
      */
      ...
      infile->GetObject("BJets_pt_"+names[i]+"_leptSel", BJets_pt_leptSel[i] );
      if(i==0) BJets_pt_leptSel[i]->SetMarkerStyle(20);  BJets_pt_leptSel[i]->SetFillColor( colors[i]); BJets_pt_leptSel[i]->Rebin(4); BJets_pt_leptSel[i]->Scale(LUMI);
      BJets_pt_leptSel[i]->GetXaxis()->SetRangeUser(0,1000);
    ...
    c[nc++] = new TCanvas(outF_name+"-BJets_pt_leptSel",outF_name+"-BJets_pt_leptSel" );  gPad->SetLogy();
    THStack s_BJets_pt_leptSel("s_BJets_pt_leptSel","s_BJets_pt_leptSel");
    for(int i=1; i<nf; i++){    s_BJets_pt_leptSel.Add(BJets_pt_leptSel[i]);  }
    BJets_pt_leptSel[0].GetXaxis()->SetTitle("BJets p_{T}"); BJets_pt_leptSel[0]->GetYaxis()->SetTitle("events");
    BJets_pt_leptSel[0]->Draw("ep");  s_BJets_pt_leptSel.Draw("hsames");  BJets_pt_leptSel[0]->Draw("samese");  l->Draw();
